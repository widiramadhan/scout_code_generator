...
''');
  ...
  final bool isListUc = hasResponse && (pascal.endsWith('s') || pascal.contains('List'));
  final String returnTypeUc = hasResponse
      ? (isListUc ? "Either<Failure, List<${pascal}Entity>>" : "Either<Failure, ${pascal}Entity>")
      : "Either<Failure, void>";
...
''';
  if (hasRequest) {
    ucInterfaceContent += "import '../../data/model/request/${snake}_request.dart';\n";
  }
  ...
  if (hasRequest) {
    ucInterfaceContent += "  Future<${returnTypeUc}> call({required ${pascal}Request request});\n";
  } else {
...
''';
  if (hasRequest) {
    ucImplContent += "    final result = await repo.$camel(request: request);\n    return result;\n";
  } else {
    ucImplContent += "    final result = await repo.$camel();\n    return result;\n";
  }
  ucImplContent += '''
  }
}
''';
  ...
  if (repoFile.existsSync()) {
    ...
    final classDecl = RegExp(r'abstract class \\w+Repository\\s*\\{').firstMatch(content);
    if (classDecl != null) {
      final classOpenBraceIdx = content.indexOf('{', classDecl.start);
      final classCloseBraceIdx = content.lastIndexOf('}');
      if (classOpenBraceIdx != -1 && classCloseBraceIdx != -1 && classCloseBraceIdx > classOpenBraceIdx) {
        existingMethods = content.substring(classOpenBraceIdx + 1, classCloseBraceIdx);
      }
    }
    // Avoid duplicates
    ...
    final existingBlock = existingMethods.trim().isEmpty
        ? ""
        : existingMethods.trimRight() + (existingMethods.endsWith('\n') ? '' : '\n');
    final newContent = imports +
